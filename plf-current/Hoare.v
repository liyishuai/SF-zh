...
(** **** 练习：1 星, standard, optional (valid_triples) 
    ...
    下列的霍尔三元组是否_'成立'_，亦即，表述的 [P]、[c]、[Q] 之间的
    关系是否为真？
...
(** 霍尔逻辑的目标是提供一种_组合的_ 方法，
    ...
    用这种组合的方式，我们甚至不用展开 [hoare_triple] 的定义也能证明
    一段程序是正确的。*)
...
(** **** 练习：3 星, advanced (hoare_asgn_fwd) 
    ...
    然而，我们可以通过引入一个_'参数'_ [m]（一个 Coq 整数）来记录 [X] 原
    来的值，来定义一个赋值的证明规则，它可以更直觉地，“正向地工作”。
    ...
    [st']。证明这个规则是成立的。（注意，这个规则比 [hoare_asgn] 复杂些。)
*)
...
(** 有的时候我们从其它证明规则中得到的前置条件和后置条件可能并不
    ...
    却不行。这个三元组是成立的，不过它并不是 [hoare_asgn] 的实例，因
...
(** 仔细考虑一下这个想法，我们可以看到对一个成立的三元组加强前置条件
    或者减弱后置条件总是能得到一个成立的三元组。这种想法可以用两条
...
(** 这又是一个来看一下 [eapply] 策略的好机会。我们在逻辑基础的
    ...
    来告诉 Coq，类似于：“耐心点儿：空缺的那部分会在证明中过会儿
    再填上。” *)
...
(** **** 练习：3 星, standard (swap_exercise) 
    ...
    它符合如下规范：
     ...
     你的证明应该不需要使用 [unfold hoare_triple]。
...
(** **** 练习：4 星, standard (if1_hoare) 
    ...
    我们推荐你，在尝试之后的练习之前，先完成这个。因为它应该会让你对这个例子
    有更加完善的理解。*)

(** 第一步是引入之前出现的命令和记号，并且加入新的命令。（我们已经
    帮你弄好了。在这里要用一个独立的模组来避免污染全局命名空间。）*)
